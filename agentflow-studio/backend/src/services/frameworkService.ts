...
export class FrameworkService {
  async executeWorkflow(prompt: string, conversationId?: string) {
    try {
      const response = await axios.post(`${FRAMEWORK_BASE_URL}/execute`, {
        prompt,
  ...
  async getAgents(): Promise<Agent[]> {
    try {
      const response = await axios.get(`${FRAMEWORK_BASE_URL}/agents`);

      const frameworkAgents = response.data.agents || [];
      return frameworkAgents.map((agent: any) => ({
        id: agent.id,
        name: agent.id,
  ...
  async getHealthStatus() {
    try {
      const response = await axios.get(`${FRAMEWORK_BASE_URL}/`);
      return {
        status: "connected",
